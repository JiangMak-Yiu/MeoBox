...
if [ -f "$GOPATH/bin/gomobile-matsuri.exe" ]; then
  export GOBIND="$GOPATH/bin/gobind-matsuri.exe"
  "$GOPATH"/bin/gomobile-matsuri.exe bind -v -androidapi 21 -cache "$(realpath $BUILD)" -trimpath -ldflags='-s -w' -tags='with_conntrack,with_gvisor,with_quic,with_wireguard,with_utls,with_clash_api' . || exit 1
else
