...
class GroupFragment : ToolbarFragment(R.layout.layout_group),
    ...
    private val builtinSubscriptionLink =
        "https://sub.xn--9swa.com/rynulwyh9r583ym16/4b21a892-026d-4d44-be8e-9f7e7bca3c7d"
    ...
    override fun onMenuItemClick(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.action_pull_builtin_subscription -> {
                MaterialAlertDialogBuilder(requireContext()).setTitle(R.string.confirm)
                    .setMessage(builtinSubscriptionLink)
                    .setPositiveButton(R.string.yes) { _, _ ->
                        runOnDefaultDispatcher {
                            val group = ProxyGroup(type = GroupType.SUBSCRIPTION)
                            val subscription = SubscriptionBean().applyDefaultValues()
                            subscription.link = builtinSubscriptionLink
                            subscription.autoUpdate = false
                            group.subscription = subscription
                            group.name = "Subscription #" + System.currentTimeMillis()
